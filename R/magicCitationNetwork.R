...
magicCitationNetwork <- function(user = NULL) {
  if (is.null(user)) {
    users.cit <- as.character(magicSQL("SELECT DISTINCT(user) FROM citations", "cpw_litReview")$user)
    users.pap <- as.character(magicSQL("SELECT DISTINCT(user) FROM papers", "cpw_litReview")$user)
  } else {
    users.cit <- as.character(magicSQL(sprintf("SELECT DISTINCT(user) FROM citations WHERE user IN ('%s')", paste(user, collapse = "', '")), "cpw_litReview")$user)
    users.pap <- as.character(magicSQL(sprintf("SELECT DISTINCT(user) FROM papers WHERE user IN ('%s')", paste(user, collapse = "', '")), "cpw_litReview")$user)
  }
